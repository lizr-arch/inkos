...
const strings = {
  ...
  "reader.localEditTitle": { zh: "局部替换（补丁）", en: "Local Replace (Patch)" },
  "reader.localEditReviewTag": { zh: "会标记需复审", en: "Marks for review" },
  "reader.localEditHint": { zh: "把正文中需要替换的原文粘贴到左侧，再把新文本粘贴到右侧。应用后会将本章状态置为「审计失败」以强制复审。", en: "Paste the exact target text from the chapter on the left, and the replacement on the right. Applying a patch marks this chapter as audit-failed so it must be reviewed again." },
  "reader.localEditTargetPlaceholder": { zh: "目标文本（必须能在正文中找到）", en: "Target text (must exist in chapter)" },
  "reader.localEditReplacementPlaceholder": { zh: "替换为（新文本）", en: "Replacement text" },
  "reader.localEditApply": { zh: "应用补丁", en: "Apply Patch" },
  "reader.localEditApplying": { zh: "应用中...", en: "Applying..." },
  "reader.inlineMarksTitle": { zh: "内联标记精修（【…】/[[…]]）", en: "Inline Marks Polish (【…】/[[…]])" },
  "reader.inlineMarksUsesModel": { zh: "会调用模型", en: "Uses model" },
  "reader.inlineMarksHint": { zh: "把当前章节正文复制一份，在需要修改的位置插入【修改要求】或[[建议：...]]标记（不要改动其他文字），然后点击应用。系统会按 spot-fix 生成补丁并写回章节。", en: "Copy the current chapter text, insert 【instruction】 or [[note]] marks at the spots you want changed (do not edit other text), then apply. The system generates spot-fix patches and writes them back." },
  "reader.inlineMarksPlaceholder": { zh: "粘贴带内联标记的完整章节内容…", en: "Paste full chapter content with inline marks…" },
  "reader.inlineMarksFill": { zh: "填入当前正文", en: "Fill from current" },
  "reader.inlineMarksApply": { zh: "生成并应用", en: "Generate & Apply" },
  "reader.inlineMarksApplying": { zh: "处理中...", en: "Processing..." },
